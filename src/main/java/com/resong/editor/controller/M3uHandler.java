package main.java.com.resong.editor.controller;
...
import main.java.com.resong.editor.interfaces.PlaylistFileHandler;
...
public class M3uHandler implements PlaylistFileHandler {
     ...
     * is incorrect/corrupt, display an error message asking if the user would like to
     * find the file and fix the error; otherwise remove the mp3 file from the playlist
                ...
                while (reader.hasNextLine()) {
                    ...
                    try {
                        ...
                        if (errorDialog.getResult() == ButtonType.OK) {
                            ...
                            FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("MP3 files (*.mp3)", "*" + SONG_EXT);
     ...
     * another file in the same directory has the same name and asks user
     * if they want to overwrite that file; if they do not, then display 
     * an error message
     * 
     * @param filePath file path of the Playlist to be renamed
     * @param newName new name of the Playlist
     * @throws IOException thrown when a playlist already exists in the same directory with the same name
     */
    @Override
    public void rename(String filePath, String newName) throws IOException {
        ...
        if (destFile.exists()) {
            ...
            if (confirmBox.getResult() == ButtonType.YES) {
                Files.move(source, source.resolveSibling(newName + PLAYLIST_EXT), java.nio.file.StandardCopyOption.REPLACE_EXISTING);
    ...
    /**
     * Helper method to display error dialog message notifying the user
     * of a corrupt file
     * 
